	...
	public void changeAllLookingRanged( int radius, Entity lookingAt, World world ) {
		for ( int i = 1; i < world.loadedEntityList.size(); i++ ) {
			...
			if ( entity.getDistanceToEntity( lookingAt ) <= radius ) {
				entity.setAngles( ( float ) VecHelper.getYaw( VecHelper.createVecFromEntity( lookingAt ), VecHelper.createVecFromEntity( entity ) ), ( float ) VecHelper.getPitch( VecHelper.createVecFromEntity( lookingAt ), VecHelper.createVecFromEntity( entity ) ) );
			}
